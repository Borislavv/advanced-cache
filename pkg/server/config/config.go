...
type Server struct {
	...
	ServerName string `envconfig:"SERVER_NAME" mapstructure:"FASTHTTP_SERVER_NAME" default:"DefLang"`
	// ServerPort is a port for shared server (endpoints like a /probe for k8s).
	ServerPort string `envconfig:"SERVER_PORT" mapstructure:"FASTHTTP_SERVER_PORT" default:":8020"`
	// ServerShutDownTimeout is a duration value before the server will be closed forcefully.
	ServerShutDownTimeout time.Duration `envconfig:"FASTHTTP_SERVER_SHUTDOWN_TIMEOUT" mapstructure:"SERVER_SHUTDOWN_TIMEOUT" default:"5s"`
	// ServerRequestTimeout is a timeout value for close request forcefully.
	ServerRequestTimeout time.Duration `envconfig:"FASTHTTP_SERVER_REQUEST_TIMEOUT" mapstructure:"SERVER_REQUEST_TIMEOUT" default:"1m"`
}
